...
*   **本我中庭**：
    ...
    *   **怒气核心**：环绕式怒气槽（10格），下沉至头像下沿，以红色“怒”字徽章显示，数值输入框有明确的可编辑反馈。
*   **资源与速度**：左侧以资源舱显示 **气血、内力**，资源数值居中、名称贴近右侧；速度以贴近头像左下侧的机动徽记显示，数据随装备/Buff实时变动。
*   **状态舱**：右侧以透明悬浮标题印章和小图标单行状态签条显示当前 Buff/Debuff，状态区可容纳两行签条；点击状态或空态徽记可打开系统状态盘，标题可独立收起/展开，无状态时保留“气机清静”空态，有状态时支持叠层、剩余时间和舱内滚动。
*   **比例与锚点**：设置面板默认值为 1；小队 HUD 在 1.1 版式基准上再次放大 10%，人物 HUD 按 1.05 版式基准定标，设置值只负责直接调整当前版式，不再叠加隐藏基准倍率。友方、敌方和底部人物 HUD 分别保持原有锚点，不挤压技能栏。
*   **操作底座**：
...
*   **状态折叠**：点击右侧“状态”标题印章，可只收起状态列表，保留标题和当前状态数量。
*   **HUD比例**：可在模组设置中分别调整小队 HUD 与人物 HUD 的显示比例。
